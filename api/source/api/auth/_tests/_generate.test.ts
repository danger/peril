import { createPerilUserJWT, getDetailsFromPerilJWT, PerilOAuthUser } from "../generate"
...
it("creates a JWT for a stubbed user", () => {
  ...
  const jwt = createPerilUserJWT(user, [])
...
it("gets data from a JWT it generated", async () => {
  ...
  const jwt = createPerilUserJWT(user, installations)
  // Just a prefix for now
